...
function CommentaryItem({ item }: CommentaryItemProps) {
    ...
    switch (item.type) {
        ...
        case 'six':
            ...
            break;
        case 'milestone':
            className += "bg-purple-900/20 border-purple-500 text-purple-400 font-bold italic";
            break;
        case 'maiden':
            className += "bg-emerald-900/20 border-emerald-500 text-emerald-400 font-bold italic";
            break;
        case 'intro':
    ...
    return (
        <div className={className}>
            {item.over && item.type !== 'intro' && (
                <span className="text-gray-500 mono mr-2 text-xs opacity-70">{item.over}</span>
            )}
            {item.text}
        </div>
    );
