	...
	return (
		<div className="min-h-screen p-6 flex flex-col items-center animate-fade-in bg-[#0B0E14] text-white">
			<div className="text-center mt-10 mb-8">
				<h2 className="text-xs uppercase tracking-[0.3em] text-amber-500 font-bold mb-2">
					Match Finished
				</h2>
				<h1 className="text-3xl font-black italic mb-2">{result}</h1>
				<p className="text-xs text-gray-400 italic mb-4">
					{GameState.tossResult.value} •{" "}
					<span className="text-gray-500">{GameState.pitch.value} Pitch</span>
				</p>
			...
			<div className="w-full max-w-2xl grid grid-cols-2 gap-4 mb-6">
				<div className="bg-[#161B22] p-4 rounded-xl border border-gray-800">
					<h3 className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-3 border-b border-gray-700 pb-2">
						Best Batters
					</h3>
					<div className="space-y-2">
						{getTopPerformers(history)
							.batters.slice(0, 3)
							.map((p, i) => (
								<div
									key={i}
								...
								>
									<span
										className={`${i === 0 ? "text-white font-bold" : "text-gray-400"}`}
									>
										{p.name}
									</span>
				...
				<div className="bg-[#161B22] p-4 rounded-xl border border-gray-800">
					<h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mb-3 border-b border-gray-700 pb-2">
						Best Bowlers
					</h3>
					<div className="space-y-2">
						{getTopPerformers(history)
							...
							.map((p, i) => (
								...
								>
									...
									<span className="font-mono text-gray-300">
										{p.bowlStats.wicketsTaken}/{p.bowlStats.runsConceded}{" "}
										<span className="text-[9px] text-gray-600">
											({Math.floor(p.bowlStats.balls / 6)}.
											{p.bowlStats.balls % 6})
										</span>
									</span>
								</div>
					...
					return (
						<div className="w-full max-w-2xl mb-6">
							<WormGraph
								...
								totalOvers={
									GameState.formatConfigs[GameState.format.value].balls / 6
								}
								color={inn1Color}
								legend={{
									main: inn1.team,
						...
						>
							...
							>
								<div className="flex items-center gap-3">
									...
									<div>
										<h3 className="font-black text-amber-500 uppercase tracking-widest text-[10px] mb-1">
											{inn.team}
										</h3>
								...
								<div className="text-right">
									<span className="mono font-black text-2xl text-white">
										{inn.score}/{inn.wickets}
									</span>
									<p className="text-[10px] text-gray-500 mono">
										{Math.floor(inn.overs / 6)}.{inn.overs % 6} OVS
									</p>
								</div>
							</div>
							...
							{isExpanded && (
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 text-xs text-gray-400 animate-fade-in-down">
									<div>
										<p className="mb-2 font-bold uppercase text-[10px] text-amber-500/80">
											Batting
										</p>
										{inn.batting.map((p) => (
											<div
												key={p.name}
												className={`flex justify-between border-b border-gray-800 py-1 ${p.batStats.out ? "text-gray-500" : "text-white"}`}
											>
												<span>
													{p.name} {p.batStats.out ? "" : "*"}
												</span>
												<span className="font-mono">
													{p.batStats.runs} ({p.batStats.balls})
												</span>
											</div>
										))}
									</div>
									<div>
										...
										{inn.bowling
											...
											.sort(
												(a, b) =>
													b.bowlStats.wicketsTaken - a.bowlStats.wicketsTaken ||
													a.bowlStats.runsConceded - b.bowlStats.runsConceded,
											)
											.map((p) => (
												<div
													key={p.name}
												...
												>
													...
													<span className="font-mono text-white">
														{p.bowlStats.wicketsTaken}-
														{p.bowlStats.runsConceded}{" "}
...
function getMatchDescription(
	...
	history.forEach((inn) => {
		inn.batting.forEach((p: Player) => {
			if (p.batStats.runs > maxImpact) {
				...
				topPerformer = {
					name: p.name,
					stat: `${p.batStats.runs}(${p.batStats.balls})`,
					type: "bat",
				};
			}
		});
		inn.bowling.forEach((p: Player) => {
			const bowlImpact =
				p.bowlStats.wicketsTaken * 25 +
				p.bowlStats.maidens * 15 -
				p.bowlStats.runsConceded;
	...
	if (totalMaidens > 0)
		description += `Defensive pressure was key with ${totalMaidens} maiden over${totalMaidens > 1 ? "s" : ""} bowled. `;
	if (topPerformer) {
...
function getTopPerformers(history: MatchHistoryEntry[]) {
	...
	history.forEach((inn) => {
		allBatters.push(
			...inn.batting.filter((p) => p.batStats.balls > 0 || p.batStats.out),
		);
	...
	allBatters.sort(
		(a, b) =>
			b.batStats.runs - a.batStats.runs || a.batStats.balls - b.batStats.balls,
	);
