...
export class SimulationEngine {
	...
	simulateBall(striker: Player, bowler: Player): BallOutcome {
		...
		const bowlerEconomy = bowler.economy || 8.0;
		...
		let aggFactor = this.battingEngine.getAggressionFactor(
			striker,
			wicketsDown,
			oversLeft,
		);
		...
		const activePhase = phases
			? phases.find((p) => currentOver >= p.start && currentOver <= p.end)
			: null;
		...
		weights[5] *=
			(config.wicketMod * bowlSkillFactor * aggFactor) / skillFactor;
		...
		if (style === "Spin") {
			// Spin Distribution
			if (randD > 0.95)
				outcome.deliveryType = "yorker"; // Faster/Arm ball
			else if (randD > 0.9)
				outcome.deliveryType = "bouncer"; // Short ball/Drag down
			else if (randD > 0.6)
				outcome.deliveryType = "full"; // Flighted
			else if (randD > 0.45)
				outcome.deliveryType = "slow"; // Slower through air
			else outcome.deliveryType = "length"; // Stock ball
		} else {
			// Pace Distribution
			if (randD > 0.85) outcome.deliveryType = "yorker";
			else if (randD > 0.7) outcome.deliveryType = "bouncer";
			else if (randD > 0.5) outcome.deliveryType = "full";
		...
		if (outcome.isWicket) {
			...
			// Wicket Type Logic
			if (outcome.deliveryType === "yorker" || outcome.timing === "missed")
			...
			if (outcome.runs === 6) {
				...
				else if (outcome.deliveryType === "bouncer")
					outcome.shotType = Math.random() > 0.5 ? "pull" : "cut";
