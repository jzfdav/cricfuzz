...
export class BattingEngine {
    ...
    getAggressionFactor(striker: Player, wicketsDown: number, oversLeft: number): number {
        ...
        if (wicketsDown >= 7) baseAgg *= 0.7; // Tailenders or collapse
        else if (wicketsDown >= 4 && oversLeft > (config.balls / 6) * 0.4) baseAgg *= 0.9; // Rebuilding
        ...
        if (oversLeft <= (totalOvers - deathPhaseStart)) {
            // Death Overs
            if (wicketsDown < 6) baseAgg *= 1.5; // Good position, smash
            else if (wicketsDown < 8) baseAgg *= 1.2; // Slog
        }

        // Target Pressure (Chasing)
        if (GameState.innings.value === 2 && GameState.target.value) {
            const rrr = (GameState.target.value - GameState.score.value) / (oversLeft || 1);
            if (rrr > 10) baseAgg *= 1.4; // Desperate
