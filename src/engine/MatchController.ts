...
export class MatchController {
	...
	startMatch(t1: string | null, t2: string | null, loopCallback: () => void) {
		...
		if (p1 && p2) {
			...
			this.addCommentary(
				`Experts are keeping a close eye on ${p1.name} and ${p2.name} today - they looked in supreme touch during the warmups!`,
				"info",
			);
		}
	...
	private addCommentary(
		...
		GameState.commentary.value = [entry, ...GameState.commentary.value].slice(
			0,
			100,
		);
	}
