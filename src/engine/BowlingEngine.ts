...
export class BowlingEngine {
	...
	private spellData: Map<
		string,
		{ currentSpellBalls: number; ballsSinceLastSpell: number }
	> = new Map();
	...
	updateSpells(lastBowlerName: string) {
		...
		if (!this.spellData.has(lastBowlerName)) {
			this.spellData.set(lastBowlerName, {
				currentSpellBalls: 0,
	...
	getSelectionScore(p: Player, currentBowler: string): number {
		...
		if (
			stats.ballsSinceLastSpell < limitInfo.minRest &&
			stats.ballsSinceLastSpell > 0
		)
			return 0;
		...
		// Define Middle Overs
		const isMiddle =
			(format === "T20" && currentOver >= 6 && currentOver < 15) ||
			(format === "ODI" && currentOver >= 10 && currentOver < 40);

		// Define Death Overs
		const isDeath =
			(format === "T20" && currentOver >= 16) ||
